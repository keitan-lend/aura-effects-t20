...
/**
 * Provided the arguments for the updateItem hook, checks whether the item carries any aura effects and, if
 * its update could have changed those effects' suppressed state (e.g. an equip/attune toggle on systems like
 * Tormenta20 or dnd5e, where suppression is driven by Item data rather than the effect itself), refreshes auras
...
async function updateItem(item, updates, options, userId) {
  ...
  if (!actor) return;
  const hasAuraEffect = item.effects?.some(e => e.type === "auraeffects.aura");
  if (!hasAuraEffect) return;
  const [token] = actor.getActiveTokens(false, true);
  if (!token) return;
  if (!checkActiveGM()) return;
  await updateAllAuraRegions(token);

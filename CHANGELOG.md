...
## 2.2.2-t20.1 (build local — compatibilidade Tormenta20 / Foundry v14)
- Corrigido bug em `queries.mjs`: ao decidir qual efeito "vence" entre múltiplas auras não-empilháveis com o mesmo nome dentro do mesmo lote, a comparação usava o documento original (`effect`) em vez dos dados já processados (`effectData`), fazendo a substituição nunca (ou incorretamente) acontecer.
- Adicionado hook `updateItem`: sistemas como o Tormenta20 (e dnd5e) suprimem efeitos com base em um campo do *Item* (ex.: `system.equipado`), não no próprio Active Effect. Sem esse hook, equipar/desequipar um item que carrega uma aura não atualizava a região/aplicação da aura automaticamente.
- Completadas as 4 chaves que faltavam em `lang/pt-BR.json` (`collisionType` e `HighlightMode`).
- Confirmado por leitura de código (não testado em jogo real): a detecção automática de compatibilidade em `registerAuraType()`/`registerAuraSheet()` já mistura corretamente com `BaseEffect` e `ActiveEffectConfigT20` do Tormenta20 — não há colisão de nomes de campo entre os ~15 campos do módulo e os 5 campos próprios do T20 (`targets`, `status`, `onuse`, `abilityUse`, `aura`).
